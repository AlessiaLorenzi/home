	...
	*flag_pointer = FALSE ;
	...
	device_msg[0] = 0 ;
		...
		( strncmp ( message , "!F", 2 ) == 0 ) )
			...
			if (riprova)
				...
				if ( ( line_ok == 0 ) || ( atoi ( device_msg ) != address[supply_number] ) )
					{
					timeout_file = fopen("./timeout_file","a");
					fprintf(timeout_file, "B ERRORE INDIRIZZO %s alim = %d ** %s ** ", tty_name,  address[supply_number], device_msg);
					chmod("./timeout_file", 0666);
					fclose(timeout_file);
	...
	// =====================================================================================================
				...
				( strncmp ( message , "!S1", 3 ) == 0 ) ) 
					...
					strcpy(answ1, device_msg);

				if ( strncmp ( message , "!VER", 4 ) == 0 )
					{
					ps_do_read();
					strcpy(answ2, device_msg);
					ps_do_read();
					strcpy(answ3, device_msg);
					}
				...
				//if ( VERBOSE ) printf("2 ** riprova = %d on_error = %d, mode = %d, addr = %d\n", 
				//riprova, on_error, alim_mode[supply_number], address[supply_number]);
				...
				if (riprova)
					...
					if ( ( line_ok == 0 ) || ( atoi ( device_msg ) != address[supply_number] ) )
						{
						timeout_file = fopen("./timeout_file","a");
						fprintf(timeout_file, "A ERRORE INDIRIZZO %s alim = %d ** %s ** ", tty_name,  address[supply_number], device_msg);
					chmod("./timeout_file", 0666);
						fclose(timeout_file);
						system("date >> timeout_file");
			...
			// inserire qui' il check sulla risposta

			if ( strncmp ( message , "!S1", 3 ) == 0 && device_msg[4] != '!')
				{
				printf(" EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEErrore in S1\n");
				ps_do_read();
				}
			if ( strncmp ( message , "!AD 0", 5 ) == 0 && ( device_msg[0] < '0' || device_msg[0] > '9' ))
				{
				printf(" EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEErrore in AD\n");
				ps_do_read();
				}
			//======================================
				if ( strncmp ( message , "!VER", 4 ) == 0 )
					{
					strcpy(device_msg, answ1);
					strcat(device_msg, answ2);
					strcat(device_msg, answ3);
					}
...
timeout_file = fopen("./timeout_file","a");
//printf("-----------7\n");
printf("FIRST  TIMEOUT: primavolta = %d on_error = %d, mode = %d, addr = %d\n", primavolta, on_error, alim_mode[supply_number], address[supply_number]) ;
fflush( stdout ) ;
timeout_flag = 1 ;
riprova = 1 ;
printf("FIRST  TIMEOUT : nlim= %d, addr= -S-%d- line is %s ** %s **\n\n\n\n", supply_number, address[supply_number],  tty_name, device_msg);
fprintf(timeout_file, "\n\nFIRST  TIMEOUT %s alim = %d ** %s ** i = %d %d  ", tty_name,  address[supply_number], device_msg,  charcnt, chi);
chmod("./timeout_file", 0666);
//printf("-----------8\n");
fclose(timeout_file);
system("date >> timeout_file");
...
timeout_file = fopen("./timeout_file","a");
//printf("-----------11\n");
printf("SECOND TIMEOUT: primavolta = %d on_error = %d, mode = %d, addr = %d\n", primavolta, on_error, alim_mode[supply_number], address[supply_number]) ;
fflush( stdout ) ;
timeout_flag = 1 ;
alim_mode[supply_number] = 0 ;
riprova = 0 ;
fprintf(timeout_file, "SECOND TIMEOUT %s alim = %d ** %s ** i = %d %d  ", tty_name,  address[supply_number], device_msg,  charcnt, chi);
//printf("-----------12\n");
printf("SECOND TIMEOUT : nlim= %d, addr= -S-%d- line is %s ** %s **\n\n\n\n", supply_number, address[supply_number],  tty_name, device_msg);
chmod("./timeout_file", 0666);
fclose(timeout_file);
system("date >> timeout_file");
//printf("-----------13\n");
