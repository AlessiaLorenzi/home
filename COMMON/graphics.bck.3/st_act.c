...
float ci, cl, vl;
char s1[20], s2[20] ,s3[20];
extern agg_etich_alarm ();
extern agg_etich_value ();



al = steerer[button[numero_bottone].obj1].alim1;
...
//	if ( (((cl-ci)/ci)*100.0 > 2.5 || ((cl-ci)/ci)*100.0 < -2.5))
	if ( (cl-ci > 0.1 || cl-ci < -0.1))
	...
	else agg_etich_value ( al, cl );

	}

al = steerer[button[numero_bottone].obj1].alim2;
sprintf ( to_server ,"st comm : ALIM = %2d #", al);
if ( verbose )
    	printf ("ALPI send: %s\n", to_server);
write( server_channel, to_server, BUF_SIZE ) ;
...
al = steerer[button[numero_bottone].obj3].alim1;
sprintf ( to_server ,"st comm : ALIM = %2d #", al);
if ( verbose )
    	printf ("ALPI send: %s\n", to_server);
write( server_channel, to_server, BUF_SIZE ) ;
...
al = steerer[button[numero_bottone].obj3].alim2;
sprintf ( to_server ,"st comm : ALIM = %2d #", al);
if ( verbose )
    	printf ("ALPI send: %s\n", to_server);
write( server_channel, to_server, BUF_SIZE ) ;
...
agg_etich_value ( alimentatore, on_off ) 
int alimentatore;
float on_off;
{
Arg args[10];
...
agg_etich_alarm ( alimentatore, on_off ) 
int alimentatore;
int on_off;
{
Arg args[10];
...
if ( steerer_supply[alimentatore].finestra == 1 ) 
	...
	if ( steerer_supply[alimentatore].x_oppure_y == 1 )
		...
		if ( on_off ) 
			{
			XtSetArg(args[0], XtNlabel, "ATTENZIONE : CORRENTE NON REGOLATA !!");
			XtSetArg(args[1], XtNforeground, pixel_red);
			}
		else
			{
			XtSetArg(args[0], XtNlabel, " ");
			XtSetArg(args[1], XtNforeground, pixel_grey);
			}
		XtSetValues(st_real_a_1 , args, 2 ) ;
		}
