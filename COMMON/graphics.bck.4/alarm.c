...
file_da_leggere = fopen("./dati_fascio/ma_sett","r");
...
if ( file_da_leggere != NULL )
	...
	if ( page_type != -1 )
		...
		if ( button[ numero_bottone ].type == 2 || button[ numero_bottone ].type == 4 ) 
			{
			printf("alarm.c: chiamo ma2q3q_refresh 1\n");
			ma2q3q_refresh() ;
			}
...
//	system("rm -f ./dati_fascio/ma_sett");
	system("mv    ./dati_fascio/ma_sett ./loaded_files/ma_sett-`date +%F_%T`");
	}

file_da_leggere = fopen("./dati_fascio/ma_sett.mag_set","r");
...
//	system("rm -f ./dati_fascio/ma_sett");
	system("mv    ./dati_fascio/ma_sett.mag_set ./loaded_files/ma_sett.mag_set-`date +%F_%T`");
	}
...
strncpy(supply[malim1].s1, from_server, 24);
...
sprintf ( to_server ,"ma comm : ALIM = %2d !AD 0", malim1);
write( server_channel, to_server, BUF_SIZE ) ;
read( server_channel, from_server, BUF_SIZE ) ;
sscanf( from_server, "%d", &adc1 ) ;
supply[malim1].real_current =  ((float)(adc1)*(float)(supply[malim1].maxcurrent)/100.0);
...
strncpy(supply[malim2].s1, from_server, 24);
...
sprintf ( to_server ,"ma comm : ALIM = %2d !AD 0", malim2);
write( server_channel, to_server, BUF_SIZE ) ;
read( server_channel, from_server, BUF_SIZE ) ;
sscanf( from_server, "%d", &adc1 ) ;
supply[malim2].real_current =  ((float)(adc1)*(float)(supply[malim2].maxcurrent)/100.0);
...
if ( ( al_1 == ON ) || ( al_2 == ON ) )
	{
	// almeno uno dei due e' in allarme  == >> ROSSO
        XtSetArg( args[0], XtNbackground, pixel_red);
        XtSetArg( args[1], XtNforeground, pixel_white);
	}
else
	if ( ( stub_1 == ON ) || ( stub_2 == ON ) )
		{	
		// almeno uno dei due e' in stub  == >> NERO
		...
		if ( ( on_1 == ON ) && ( on_2 == ON ) )
			...
			if ( ( on_1 == ON ) || ( on_2 == ON ) )
				{
				// almeno uno dei due e' on  == >> GIALLO
                                XtSetArg( args[0], XtNbackground,pixel_yellow);
                                XtSetArg( args[1], XtNforeground, pixel_black);
				}
			else
				{
				// tutti e due off  == >> BIANCO
                           	XtSetArg( args[0], XtNbackground, pixel_white);
                           	XtSetArg( args[1], XtNforeground, pixel_black );
				}	
