...
char stringa[100];
    ...
    for (cnt = 0; cnt<100; cnt++)
        ...
        if ( cnt > 0 ) for ( cnt2 = 0 ; cnt2 < 100 ; cnt2++ )
		{
                if ( magnet[cnt2].alim1 == cnt )
                       {
        	       if ( supply[cnt].on_off == 0 ) fprintf(file_to_write_cur, "%2d   %2d   %10s   %10.6f   %10.6f   OFF  %24s\n",cnt, cnt2, magnet[cnt2].n_alim1, supply[cnt].current, supply[cnt].real_current, supply[cnt].s1);
        	       if ( supply[cnt].on_off == 1 ) fprintf(file_to_write_cur, "%2d   %2d   %10s   %10.6f   %10.6f   ON   %24s\n",cnt, cnt2, magnet[cnt2].n_alim1, supply[cnt].current, supply[cnt].real_current, supply[cnt].s1);
        	       if ( supply[cnt].on_off == 2 ) fprintf(file_to_write_cur, "%2d   %2d   %10s   %10.6f   9999999.99   STUB ........................\n",cnt, cnt2, magnet[cnt2].n_alim1, supply[cnt].current);
                       }
                if ( magnet[cnt2].alim2 == cnt )
                       {
        	       if ( supply[cnt].on_off == 0 ) fprintf(file_to_write_cur, "%2d   %2d   %10s   %10.6f   %10.6f   OFF  %24s\n",cnt, cnt2, magnet[cnt2].n_alim2, supply[cnt].current, supply[cnt].real_current, supply[cnt].s1);
        	       if ( supply[cnt].on_off == 1 ) fprintf(file_to_write_cur, "%2d   %2d   %10s   %10.6f   %10.6f   ON   %24s\n",cnt, cnt2, magnet[cnt2].n_alim2, supply[cnt].current, supply[cnt].real_current, supply[cnt].s1);
        	       if ( supply[cnt].on_off == 2 ) fprintf(file_to_write_cur, "%2d   %2d   %10s   %10.6f   9999999.99   STUB ........................\n",cnt, cnt2, magnet[cnt2].n_alim2, supply[cnt].current);
                       }
                }
...
sprintf(stringa, "%s%s%s", "./dati_fascio/", string, ".mag_set");
	...
	while ( fgets ( rigaletta, 400, file_to_read_cur) != NULL )
		{
		sscanf(rigaletta, "%d   %d   %s   %f   %f   %s  %s", &n1, &n2, name,  &rd_curr, &real_curr, status, st1 );
		//printf("\n\n\n\n*%s*\n", rigaletta);	
 		//printf("%2d   %2d   %10s   %10.6f   %10.6f   %s  %24s\n",n1, n2, name, rd_curr, real_curr, status, st1 );
