...
int cnt2;
FILE *file_to_write;
FILE *file_to_write_cur;
FILE *file_to_print;
char stringa[100];
    sprintf(stringa, "%s%s", "./dati_fascio/", string);
    file_to_write = fopen(stringa,"w");
    strcat(stringa, ".mag_set");
    file_to_write_cur = fopen(stringa,"w");

// scrittura file vecchio per i magneti
    for (cnt = 0; cnt<100; cnt++)
        {
...
// scrittura file nuovo per i magneti
for (cnt = 0 ; cnt<100 ; cnt++ ) /*questo e' un loop sul numero dei magneti*/
        {
        if ( magnet[cnt].alim1 > 0 && magnet[cnt].alim1 < 100 )
            	{
       	       	if ( supply[magnet[cnt].alim1].on_off == 0 ) fprintf(file_to_write_cur, "MA %2d   %10s   %10.6f   %10.6f   OFF  %24s\n",cnt, magnet[cnt].n_alim1, supply[magnet[cnt].alim1].current, supply[magnet[cnt].alim1].real_current, supply[magnet[cnt].alim1].s1);
       	       	if ( supply[magnet[cnt].alim1].on_off == 1 ) fprintf(file_to_write_cur, "MA %2d   %10s   %10.6f   %10.6f   ON   %24s\n",cnt, magnet[cnt].n_alim1, supply[magnet[cnt].alim1].current, supply[magnet[cnt].alim1].real_current, supply[magnet[cnt].alim1].s1);
       	       	if ( supply[magnet[cnt].alim1].on_off == 2 ) fprintf(file_to_write_cur, "MA %2d   %10s   %10.6f   9999999.99   STUB ........................\n",cnt, magnet[cnt].n_alim1, supply[magnet[cnt].alim1].current);
	    	}
        if ( magnet[cnt].alim2 > 0 && magnet[cnt].alim2 < 100 )
            	{
       	       	if ( supply[magnet[cnt].alim2].on_off == 0 ) fprintf(file_to_write_cur, "MA %2d   %10s   %10.6f   %10.6f   OFF  %24s\n",cnt, magnet[cnt].n_alim2, supply[magnet[cnt].alim2].current, supply[magnet[cnt].alim2].real_current, supply[magnet[cnt].alim2].s1);
       	       	if ( supply[magnet[cnt].alim2].on_off == 1 ) fprintf(file_to_write_cur, "MA %2d   %10s   %10.6f   %10.6f   ON   %24s\n",cnt, magnet[cnt].n_alim2, supply[magnet[cnt].alim2].current, supply[magnet[cnt].alim2].real_current, supply[magnet[cnt].alim2].s1);
       	       	if ( supply[magnet[cnt].alim2].on_off == 2 ) fprintf(file_to_write_cur, "MA %2d   %10s   %10.6f   9999999.99   STUB ........................\n",cnt, magnet[cnt].n_alim2, supply[magnet[cnt].alim2].current);
	    	}
	}

// scrittura file vecchio per gli steerers
    for (cnt = 0; cnt<100; cnt++)
        {
...
// scrittura file nuovo per gli steerers
for (cnt = 0 ; cnt<100 ; cnt++ ) 
        {
	if ( steerer[cnt].alim1 > 0 || steerer[cnt].alim2 > 0) 
            	{
		//printf("ST %2d %15s %2d %2d %6.4f %6.4f\n", steerer[cnt].steerer, steerer[cnt].name, steerer[cnt].alim1, steerer[cnt].alim2, steerer_supply[steerer[cnt].alim1].current, steerer_supply[steerer[cnt].alim2].current );
		fprintf(file_to_write_cur, "ST %2d %15s %2d %2d %6.4f %6.4f\n", steerer[cnt].steerer, steerer[cnt].name, steerer[cnt].alim1, steerer[cnt].alim2, steerer_supply[steerer[cnt].alim1].current, steerer_supply[steerer[cnt].alim2].current );
		}
	}


    fclose(file_to_write);
    fclose(file_to_write_cur);
}
...
int n1, n2;
char name[20];
float  wr_curr, rd_curr, real_curr ;
char status[40], st1[40];;
Arg arg[8];
int trovato;
int st_alim1, st_alim2;
float st_cur1, st_cur2;


sprintf(stringa, "%s%s%s", "./dati_fascio/", string, ".mag_set");

    if (file_to_read_cur = fopen(stringa, "r"))
	{
	sprintf(warn_string, "LOADED FILE : %s%s", string, ".mag_set");
	XtSetArg( arg[0], XtNforeground, pixel_black );
	XtSetArg( arg[1], XtNbackground, pixel_red );
	XtSetArg( arg[2], XtNlabel, warn_string);
	XtSetValues( mawarning_lab, &arg, 3 ) ;
	...
	XtSetArg( arg[1], XtNbackground, pixel_brown );
	XtSetValues( mawarning_lab, &arg, 2 ) ;
	XtSetValues( mawarning_but, &arg, 2 ) ;

	while ( fgets ( rigaletta, 400, file_to_read_cur) != NULL )
		{
		/////
		if ( strncmp ( rigaletta, "MA", 2 ) == 0 )
			{
			sscanf(rigaletta + 2, "%d   %s   %f   %f   %s  %s", &n1, name,  &rd_curr, &real_curr, status, st1 );
			//printf("\n\n\n\n*%s*\n", rigaletta);	
 			//printf("%2d   %10s   %10.6f   %10.6f   %s  %24s\n", n1, name, rd_curr, real_curr, status, st1 );

			trovato = 0 ;

			for (cnt = 0 ; cnt<100 ; cnt++ )
				{
				// loop sugli alimentatori
				if (  magnet[cnt].alim1 > 0 && magnet[cnt].alim1 < 9999  )
                			{
					if  ( strcmp ( name, magnet[cnt].n_alim1) == 0 )
						{
						// alimentatore TOVATO
						trovato = 1 ;
						oldvalue=supply[magnet[cnt].alim1].on_off;
						oldcurrent=supply[magnet[cnt].alim1].current;
	                			supply[magnet[cnt].alim1].current = rd_curr;
						if ( strncmp ( status, "ON",   2 ) == 0 ) supply[magnet[cnt].alim1].on_off = ON ;	
						if ( strncmp ( status, "OFF",  2 ) == 0 ) supply[magnet[cnt].alim1].on_off = OFF ;	
						if ( strncmp ( status, "STUB", 2 ) == 0 ) supply[magnet[cnt].alim1].on_off = STUB ;	
						if ( supply[magnet[cnt].alim1].on_off == oldvalue )
							{
							// lo stato non e' cambiato
							//printf("1 %d %d da     caricare %s\n",cnt, magnet[cnt].alim1, name);
	
		                			value_to_send = (int)(supply[magnet[cnt].alim1].current * 1000000.0 / supply[magnet[cnt].alim1].maxcurrent);
		                			supply[magnet[cnt].alim1].wa_current = value_to_send ;
		                			sprintf ( to_server ,"ma comm : ALIM = %2d !WA %06d", magnet[cnt].alim1, value_to_send);
							...
							{
							// lo stato e' cambiato
							//printf("1 %d %d da non caricare %s\n",cnt, magnet[cnt].alim1, name);
							supply[magnet[cnt].alim1].on_off = oldvalue;
							supply[magnet[cnt].alim1].current = oldcurrent;
							sprintf(stringa, " %s", magnet[cnt].n_alim1);
				...
				if (  magnet[cnt].alim2 > 0 && magnet[cnt].alim2 < 9999  )
                			{
					if  ( strcmp ( name, magnet[cnt].n_alim2) == 0 )
						{
						// alimentatore TOVATO
						trovato = 1 ;
						oldvalue=supply[magnet[cnt].alim2].on_off;
						oldcurrent=supply[magnet[cnt].alim2].current;
	                			supply[magnet[cnt].alim2].current = rd_curr;
						if ( strncmp ( status, "ON",   2 ) == 0 ) supply[magnet[cnt].alim2].on_off = ON ;	
						if ( strncmp ( status, "OFF",  2 ) == 0 ) supply[magnet[cnt].alim2].on_off = OFF ;	
						if ( strncmp ( status, "STUB", 2 ) == 0 ) supply[magnet[cnt].alim2].on_off = STUB ;	
						if ( supply[magnet[cnt].alim2].on_off == oldvalue )
							{
							// lo stato non e' cambiato
							//printf("2 %d %d da     caricare %s\n",cnt, magnet[cnt].alim2, name);
	
		                			value_to_send = (int)(supply[magnet[cnt].alim2].current * 1000000.0 / supply[magnet[cnt].alim2].maxcurrent);
		                			supply[magnet[cnt].alim2].wa_current = value_to_send ;
		                			sprintf ( to_server ,"ma comm : ALIM = %2d !WA %06d", magnet[cnt].alim2, value_to_send);
		                			write( server_channel, to_server, BUF_SIZE ) ;
		                			read( server_channel, from_server, BUF_SIZE ) ;
		                			sscanf( from_server, "%d",&corrente);
							...
							{
							// lo stato e' cambiato
							//printf("2 %d %d da non caricare %s\n",cnt, magnet[cnt].alim2, name);
							supply[magnet[cnt].alim2].on_off = oldvalue;
							supply[magnet[cnt].alim2].current = oldcurrent;
							sprintf(stringa, " %s", magnet[cnt].n_alim2);
							strcat(warn_string, stringa);
							XtSetArg( arg[0], XtNbackground, pixel_red );
							XtSetArg( arg[1], XtNforeground, pixel_black );
							XtSetValues( mawarning_lab, &arg, 2 ) ;
		...
		if ( strncmp ( rigaletta, "ST", 2 ) == 0 )
			{
			sscanf(rigaletta + 2, "%d   %s   %d  %d  %f   %f", &n1, name, &st_alim1, &st_alim2, &st_cur1, &st_cur2 );
			//printf("\n\n\n\n*%s*\n", rigaletta);	
			//printf( "%d   %s   %d  %d  %f   %f\n", n1, name, st_alim1, st_alim2, st_cur1, st_cur2 );
			...
			for (cnt = 0 ; cnt<100 ; cnt++ )
				{
				// loop sugli steerers
				if (  strcmp ( steerer[cnt].name, name ) == 0 )
                			{
					steerer_supply[steerer[cnt].alim1].current = st_cur1 ;
					steerer_supply[steerer[cnt].alim2].current = st_cur2 ;

					printf("%d %f \n", steerer[cnt].alim1, steerer_supply[steerer[cnt].alim1].current);
					printf("%d %f \n", steerer[cnt].alim2, steerer_supply[steerer[cnt].alim2].current);

					st_current_cmd(steerer[cnt].alim1, steerer_supply[steerer[cnt].alim1].current);
					st_current_cmd(steerer[cnt].alim2, steerer_supply[steerer[cnt].alim2].current);
					}
				}
			}
		////////
	...
	if ( file_to_read != NULL )
		...
		for (cnt = 0 ; cnt<100 ; cnt++ )
			...
			//	printf ("modificato valore di %6s (%d)\n", magnet[cnt].n_alim2, magnet[cnt].alim2 );
		                ...
		                value_to_send = (int)(supply[magnet[cnt].alim2].current * 1000000.0 / 
		...
		{
	        printf ("\007WARNING : FILE DOES NOT EXIST \n");
		sprintf(warn_string, "NOT LOADED FILE : %s", string);
	       	XtSetArg( arg[0], XtNforeground, pixel_black );
	       	XtSetArg( arg[1], XtNbackground, pixel_red );
		XtSetArg( arg[2], XtNlabel, warn_string);
		XtSetValues( mawarning_lab, &arg, 3 ) ;
...
sprintf ( to_server ,"ma comm : ALIM = %2d !F", supply_number);
write( server_channel, to_server, BUF_SIZE ) ;
read( server_channel, from_server, BUF_SIZE ) ;
...
sleep(1);
	    ...
	    strncpy(supply[supply_number].s1, from_server, 24);
            }
...
if ( button[ numero_bottone ].type == 2 ||
     button[ numero_bottone ].type == 4 ) 
	{
	printf("callbacks.c: chiamo ma2q3q_refresh 7\n")	;
	ma2q3q_refresh() ;
	}
...
if ( button[ numero_bottone ].type == 2 ||
     button[ numero_bottone ].type == 4 ) 
	{
	printf("callbacks.c: chiamo ma2q3q_refresh 8\n")	;
	ma2q3q_refresh() ;
	}
