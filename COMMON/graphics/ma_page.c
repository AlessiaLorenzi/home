...
extern int pixel_yellow, pixel_black, pixel_black ;
...
rect_onwidget( main_warn_lab, 0, 0, 180, 40, pixel_yellow ) ;
...
rect_onwidget( main_warn_lab, 0, 0, 270, 40, pixel_yellow ) ;
...
/* ANDREA: call back di fast_save_but: ON di tutti i p.s. */
void fast_save_act( fast_save_but, tag, fast_save_dat )
    Widget fast_save_but ;
    caddr_t tag ; /* not used */
    caddr_t fast_save_dat ; /* not used */
{
masave_data("fast_saved_data"); 
}

/* ANDREA: call back di fast_load_but: ON di tutti i p.s. */
void fast_load_act( fast_load_but, tag, fast_load_dat )
    Widget fast_load_but ;
    caddr_t tag ; /* not used */
    caddr_t fast_load_dat ; /* not used */
{
maread_data("fast_saved_data"); 
}

/* ANDREA: call back di maallon_but: ON di tutti i p.s. */
void maallon_act( maallon_but, tag, maallon_dat )
    Widget maallon_but ;
    caddr_t tag ; /* not used */
    caddr_t maallon_dat ; /* not used */
{
extern int verbose ;
extern Widget main_warn_lab;
extern int pixel_yellow, pixel_black, pixel_black ;
...
rect_onwidget( main_warn_lab, 0, 0, 180, 40, pixel_yellow ) ;
text_onwidget( main_warn_lab, 10, 27, " all p.s. on  ",22,
               pixel_black ) ;
...
for (cnt = 0 ; cnt<100 ; cnt++ )
	...
	if (  magnet[cnt].alim1 > 0 && magnet[cnt].alim1 < 9999  )
                {
//		sprintf ( to_server ,"ma comm : ALIM = %2d !F", magnet[cnt].alim1);
//   		write( server_channel, to_server, BUF_SIZE ) ;
//       		read( server_channel, from_server, BUF_SIZE ) ;
//sleep(3);
		sprintf ( to_server ,"ma comm : ALIM = %2d !N", magnet[cnt].alim1);
		gettimeofday(&t_start, NULL);
   		write( server_channel, to_server, BUF_SIZE ) ;
       		read( server_channel, from_server, BUF_SIZE ) ;
		gettimeofday(&t_finish, NULL);
		elapsed = 1000.0 * (t_finish.tv_sec - t_start.tv_sec + (t_finish.tv_usec - t_start.tv_usec) / 1.e6);
 		//printf("%2d  -- %8s elapsed %7.2f ms. %s\n", cnt, magnet[cnt].n_alim1, elapsed, from_server);
 		printf(" %8s %7.2f \n%s\n\n\n", magnet[cnt].n_alim1, elapsed, from_server);

            sprintf( to_server, "ma comm : ALIM = %2d !S1", magnet[cnt].alim1);
            write(server_channel, to_server, BUF_SIZE ) ;
            read(server_channel, from_server, BUF_SIZE ) ;
            if (from_server[0] ==  '.' ) supply[magnet[cnt].alim1].on_off = ON; 
            if (from_server[0] ==  '!' ) supply[magnet[cnt].alim1].on_off = OFF; 
            if (from_server[0] ==  'S' ) supply[magnet[cnt].alim1].on_off = STUB; 
	    strncpy(supply[magnet[cnt].alim1].s1, from_server, 24);

                }

	if (  magnet[cnt].alim2 > 0 && magnet[cnt].alim2 < 9999  )
                {
//		sprintf ( to_server ,"ma comm : ALIM = %2d !F", magnet[cnt].alim2);
//   		write( server_channel, to_server, BUF_SIZE ) ;
//       		read( server_channel, from_server, BUF_SIZE ) ;
//sleep(3);
		sprintf ( to_server ,"ma comm : ALIM = %2d !N", magnet[cnt].alim2);
		gettimeofday(&t_start, NULL);
   		write( server_channel, to_server, BUF_SIZE ) ;
       		read( server_channel, from_server, BUF_SIZE ) ;
		gettimeofday(&t_finish, NULL);
		elapsed = 1000.0 * (t_finish.tv_sec - t_start.tv_sec + (t_finish.tv_usec - t_start.tv_usec) / 1.e6);
 		//printf("%2d  -- %8s elapsed %7.2f ms. %s\n", cnt, magnet[cnt].n_alim2, elapsed, from_server);
 		printf(" %8s %7.2f \n%s\n\n\n", magnet[cnt].n_alim2, elapsed, from_server);

            sprintf( to_server, "ma comm : ALIM = %2d !S1", magnet[cnt].alim2);
            write(server_channel, to_server, BUF_SIZE ) ;
            read(server_channel, from_server, BUF_SIZE ) ;
            if (from_server[0] ==  '.' ) supply[magnet[cnt].alim2].on_off = ON; 
            if (from_server[0] ==  '!' ) supply[magnet[cnt].alim2].on_off = OFF; 
            if (from_server[0] ==  'S' ) supply[magnet[cnt].alim2].on_off = STUB; 
	    strncpy(supply[magnet[cnt].alim2].s1, from_server, 24);
...
extern int verbose ;
extern Widget main_warn_lab;
extern int pixel_yellow, pixel_black, pixel_black, pixel_brown ;
...
//steerer
			...
			if ( button[cnt].type == 2 )
				{
				steerer_supply[steerer[button[cnt].obj1].alim1].current = 0 ;
				st_current_cmd(steerer[button[cnt].obj1].alim1, 0 );
				steerer_supply[steerer[button[cnt].obj1].alim2].current = 0 ;
				st_current_cmd(steerer[button[cnt].obj1].alim2, 0 );
				steerer_supply[steerer[button[cnt].obj3].alim1].current = 0 ;
				st_current_cmd(steerer[button[cnt].obj3].alim1, 0 );
				steerer_supply[steerer[button[cnt].obj3].alim2].current = 0 ;
				st_current_cmd(steerer[button[cnt].obj3].alim2, 0 );
				}
			}
...
extern int pixel_yellow, pixel_black,
pixel_grey, pixel_coral, pixel_black ;
Arg args[10];
char string[100];
FILE *firmware_file;
...
firmware_file = fopen("./firmware_versions", "w");
...
for (cnt = 0 ; cnt<100 ; cnt++ ) /*questo e' un loop sul numero dei magneti*/
        ...
        if ( magnet[cnt].alim1 > 0 && magnet[cnt].alim1 < 100 )
            ...
            rect_onwidget( main_warn_lab, 0, 0, 270, 40, pixel_yellow ) ;
            text_onwidget( main_warn_lab, 20, 27, string, 15, pixel_black ) ;


		sprintf ( to_server ,"ma comm : ALIM = %2d !VER", magnet[cnt].alim1);
   		write( server_channel, to_server, BUF_SIZE ) ;
       		read( server_channel, from_server, BUF_SIZE ) ;
 		fprintf(firmware_file, " %8s \n%s\n", magnet[cnt].n_alim1, from_server);
...
//            strcpy(ma_suppl_status[magnet[cnt].alim1],from_server);
		...
		sprintf ( to_server ,"ma comm : ALIM = %2d !VER", magnet[cnt].alim2);
   		write( server_channel, to_server, BUF_SIZE ) ;
       		read( server_channel, from_server, BUF_SIZE ) ;
 		fprintf(firmware_file, " %8s \n%s\n", magnet[cnt].n_alim2, from_server);
...
//rect_onwidget( main_warn_lab, 0, 0, 180, 40, pixel_yellow ) ;
//text_onwidget( main_warn_lab, 10, 27, "reading data from steerers",22, pixel_black ) ;
        ...
        if ( button[cnt].type == 2 )
            ...
            if ( button[cnt].obj3 != 9999 &&
               ...
               strcat(string, steerer[button[cnt].obj3].name);
                ...
                rect_onwidget( main_warn_lab, 0, 0, 270, 40, pixel_yellow ) ;
                text_onwidget( main_warn_lab, 20, 27, string, 18, pixel_black ) ;
...
void open_mapage()
{
    ...
    */
    ...
    fast_load_but = XtCreateWidget( "fast_load_but",
                                     commandWidgetClass,
                                     macom_box, NULL, 0 ) ;
    XtAddCallback( fast_load_but, XtNcallback, fast_load_act, NULL ) ;

    fast_save_but = XtCreateWidget( "fast_save_but",
                                     commandWidgetClass,
                                     macom_box, NULL, 0 ) ;
    XtAddCallback( fast_save_but, XtNcallback, fast_save_act, NULL ) ;

    maallon_but = XtCreateWidget( "maallon_but",
                                     commandWidgetClass,
                                     macom_box, NULL, 0 ) ;
    XtAddCallback( maallon_but, XtNcallback, maallon_act, NULL ) ;
    ...
    XtSetValues( maremall_but, &arg, 1 ) ;
    XtSetValues( maresetall_but, &arg, 1 ) ;
    XtSetValues( fast_save_but, &arg, 1 ) ;
    XtSetValues( fast_load_but, &arg, 1 ) ;
    XtSetValues( maallon_but, &arg, 1 ) ;
    XtSetValues( mazecurall_but, &arg, 1 ) ;
    ...
    XtSetValues( maallon_but, &arg, 1 ) ;
    XtManageChild( maallon_but ) ;

    XtSetArg( arg, XtNfromHoriz, maallon_but ) ;
    XtSetValues( maresetall_but, &arg, 1 ) ;
    ...
    XtSetArg( arg, XtNfromHoriz, maexit_but ) ;
    XtSetValues( fast_save_but, &arg, 1 ) ;
    XtManageChild( fast_save_but ) ;

    XtSetArg( arg, XtNfromHoriz, fast_save_but ) ;
    XtSetValues( fast_load_but, &arg, 1 ) ;
    XtManageChild( fast_load_but ) ;
