...
extern ma2q3q_I_right();
extern ma2q3q_I_left();
extern ma2q3q_D_right();
extern ma2q3q_D_left();


extern int ma2q3q_step_Ik ;
extern int ma2q3q_min_wa_Ik ;
extern int ma2q3q_max_wa_Ik ;

extern int ma2q3q_step_Dk ;
extern int ma2q3q_min_wa_Dk ;
static int ma2q3q_max_wa_Dk ;
...
//printf("page type = %d, magnet[button[numero_bottone].obj2].alim1 = %d magnet[button[numero_bottone].obj2].alim2 = %d\n", page_type, magnet[button[numero_bottone].obj2].alim1, magnet[button[numero_bottone].obj2].alim2 );
	...
	switch(code)
	{
		case KNOB_A:
			...
			if (page_type > 0 && ( button[ numero_bottone ].type == 2 || button[ numero_bottone ].type == 4 )  )
				{
					...
					{
					printf(" %d DOWN\n",data - knob_a_old );
					ma2q3q_I_left( button[numero_bottone].obj2 );
					}
					else   
					{
					printf(" %d UP\n", data - knob_a_old );
					ma2q3q_I_right( button[numero_bottone].obj2 );
					}
			...
			if (page_type > 0 && ( button[ numero_bottone ].type == 2 || button[ numero_bottone ].type == 4 )  )
				{
					...
					{
					printf(" %d DOWN\n",data - knob_b_old );
					ma2q3q_D_left( button[numero_bottone].obj2 );
					}
					else   
					{
					printf(" %d UP\n", data - knob_b_old );
					ma2q3q_D_right( button[numero_bottone].obj2 );
					}
                			/* turn on the green light */
                			semop_retval = semctl( semaphore_id,  0, SETVAL, 1 ) ;
					}
		...
		case SWITCH_A:
			...
			printf("switch A  %d\n",data);
            		...
            		if ( data == 1 ) ma2q3q_step_Ik = 25 ;
            		if ( data == 2 ) ma2q3q_step_Ik = 10 ;
            		if ( data == 4 ) ma2q3q_step_Ik = 5 ;
            		if ( data == 8 ) ma2q3q_step_Ik = 2 ;
		...
		case SWITCH_B:
			...
			printf("switch B  %d\n",data);
            		...
            		if ( data == 1 ) ma2q3q_step_Dk = 25 ;
            		if ( data == 2 ) ma2q3q_step_Dk = 10 ;
            		if ( data == 4 ) ma2q3q_step_Dk = 5 ;
            		if ( data == 8 ) ma2q3q_step_Dk = 2 ;
