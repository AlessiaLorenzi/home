...

{
    ...
    /* open line towards teslameter */
    tesla_fildes = open(tesla_tty_name, O_RDWR ) ;
    if (tesla_fildes == -1)
        {
        printf ("\007*** errore nel primo open\n" ) ; 
        tesla_fildes = open(tesla_tty_name, O_RDWR ) ;
    ...
    ret_val = tcgetattr( tesla_fildes, &tty_termios ) ;
    if (ret_val == -1) {
        printf( "%s error (dev get attr)\n", myname ) ;
        return ;
        }
    ...
    ret_val = tcsetattr( tesla_fildes, TCSANOW, &tty_termios ) ;
    if (ret_val == -1) {
        printf( "%s error (dev set attr)\n", myname ) ;
        return ;
        }

    tesla_rs232r = fdopen( tesla_fildes, "r" ) ;
    if ( tesla_rs232r == NULL ) {
        printf( "\n%s: error opening tty for reading\n", myname ) ;
        exit( 0 ) ;
        }
    tesla_rs232w = fdopen( tesla_fildes, "w" ) ;
    if ( tesla_rs232w == NULL ) {
        printf( "\n%s: error opening tty for writing\n", myname ) ;
        exit( 0 ) ;
        }
    sleep(1) ;
