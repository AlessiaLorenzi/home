From:	SMTP%"chiara@softeco.it" 27-MAY-1997 16:56:06.14
To:	BATTISTELLA
CC:	
Subj:	Applicazione di test

Sender: chiara@softeco.it
Message-ID: <338AFCD8.6F@softeco.it>
Date: Tue, 27 May 1997 16:25:12 +0100
From: Chiara Siffredi <chiara@softeco.it>
Organization: Softeco Sismat
X-Mailer: Mozilla 3.01Gold (X11; I; SunOS 5.5.1 sun4u)
MIME-Version: 1.0
To: BATTISTELLA@lnl.infn.it
Subject: Applicazione di test
Content-Type: multipart/mixed; boundary="------------579E7E3F62F4"
...
Egr. Dott. Battistella,

come d'accordo Le invio i files relativi all'applicazione di test del
prodotto BuilderXcessory.
I files ( bxutils-c.c, callbacks-c.c, creation-c.c, creation-c.h,
main-c.c , makefile e uil.uil) sono suddivisi dai tag /* inizio SOFTECO
*/ e /* fine SOFTECO */.

Restando a Sua disposizione per ogni ulteriore chiarimento, colgo
l'occasione per porgere i miei migliori saluti.

Chiara Siffredi

PS : Purtroppo non sono riuscita a mettere i files come corpo del
messaggio. Se ci fossero dei problemi nella lettura di FILES, La
pregherei di contattarmi.

--------------579E7E3F62F4
Content-Type: text/plain; charset=us-ascii; name="FILES"
Content-Transfer-Encoding: 7bit
...
if(strcmp(input,"A")==0) {
  if (XtIsManaged(finestraFA))
    XtUnmanageChild(finestraFA);
  else
    XtManageChild(finestraFA);
  XtUnmanageChild(finestraFC);
  XtUnmanageChild(finestraFB);
} else if(strcmp(input,"B")==0) {
  if (XtIsManaged(finestraFB))
    XtUnmanageChild(finestraFB);
  else
  ...
  if (XtIsManaged(finestraFC))
    XtUnmanageChild(finestraFC);
  else
    XtManageChild(finestraFC);
  XtUnmanageChild(finestraFA);
  XtUnmanageChild(finestraFB);
}
}
...
/* fine SOFTECO */
/* inizio SOFTECO */
MOTIFINCLUDES=-I/usr/dt/share/include
XINCLUDES=-I/usr/openwin/include
OTHER_INCLUDES=-I/opt/dt/share/include
DBINCLUDES=-I$(DXHOME)/usr/include
INCLUDES= -I/ICS/epak25/include  -I. $(OTHER_INCLUDES)  $(DBINCLUDES) $(MOTIFINCLUDES) $(XINCLUDES)

CC=gcc -g
CDEBUGFLAGS=
DEFINES=
CCOPTIONS=-I/usr/openwin/include
CFLAGS=$(CDEBUGFLAGS) $(CCOPTIONS) $(DEFINES) $(INCLUDES)

MOTIFLIBS=-lXm
XLIBS=-lXt -lXext -lX11
SYSLIBS= -lm -lnsl -lsocket -lgen
CXXLIB = -lC

# Sybase libraries
LOCAL_LIBRARIES=-L/usr/openwin/lib -lXm -lXt -lX11 -lnsl -lsocket -lgen

LIBS= $(LOCAL_LIBRARIES)  $(EPAKLIBS) $(MOTIFLIBS) $(XLIBS)

OFILES = main-c.o creation-c.o callbacks-c.o  bxutils-c.o
FILES = main-c.c creation-c.c callbacks-c.c  bxutils-c.c
RM  = rm -f
...
main-c: $(OFILES) $(APP_OFILES)
	$(CC) $(CCOPTIONS) $(OFILES) $(APP_OFILES) $(LIBS) -o $@
...
!(BX) bx_info("java_excludeungenerated", "false")
...
!(BX) bx_info("vk_libs", "-L${OPENWINHOME}/lib -lvk -lvkmsg -lvkhelp -ltt -lce -lXm -lXpm -lXt -lX11"&
!(BX)	" -lsocket -lnsl -lgen")
!(BX) bx_info("vk_cflags", "-DFUNCPROTO -DXTFUNCPROTO -I${OPENWINHOME}/include/desktop")
!(BX) bx_info("vk_cc", "CC")
!(BX) bx_info("vk_excludeungenerated", "false")
...
!(BX) bx_info("c++_libs", "-L${OPENWINHOME}/lib -lXm -lXt -lX11 -lsocket -lnsl -lgen")
!(BX) bx_info("c++_cflags", "-DFUNCPROTO -DXTFUNCPROTO -I${OPENWINHOME}/include")
!(BX) bx_info("c++_cc", "CC")
...
!(BX) bx_info("c_libs", "-L${OPENWINHOME}/lib -lXm -lXt -lX11 -lsocket -lnsl -lgen")
!(BX) bx_info("c_cflags", "-I${OPENWINHOME}/include")
!(BX) bx_info("c_cc", "cc")
...
!(BX) bx_info("uil_libs", "-L${OPENWINHOME}/lib -lMrm -lXm -lXt -lX11 -lsocket -lnsl -lgen")
!(BX) bx_info("uil_uilflags", "-w -I${OPENWINHOME}/include/uil")
!(BX) bx_info("uil_cflags", "-I${OPENWINHOME}/include")
...
!(BX) bx_info("uil_uil", "uil.uil", true)
...
!(BX) bx_info("system_directory", "/opt/bxpro")
...
identifier BXG_finestraFA;
identifier BXG_finestraFB;
identifier BXG_finestraFC;
procedure BxExitCB(integer);
procedure AttivaFinestra(string);
...
!(BX)    };
!(BX)    controls {
!(BX)         unmanaged XmBulletinBoardDialog finestraFC;
!(BX)    };
!(BX)    callbacks {
!(BX)    };
!(BX)};

object finestraFC : XmBulletinBoardDialog widget {
    arguments {
        XmNautoUnmanage = false;
        XmNresizePolicy = XmRESIZE_GROW;
        XmNdialogTitle = compound_string("FC");
        XmNx = 174;
        XmNy = 156;
    };
    controls {
         managed XmScrollBar scrollBar1;
         managed XmPushButton pushButton7;
         managed XmPushButton pushButton8;
    };
    callbacks {
        MrmNcreateCallback = procedure BxGlobalWidgetId(BXG_finestraFC);
    };
};

object pushButton8 : XmPushButton widget {
    arguments {
        XmNlabelString = compound_string("H");
        XmNx = 20;
        XmNy = 80;
        XmNwidth = 40;
        XmNheight = 50;
...
object scrollBar1 : XmScrollBar widget {
    arguments {
...
!(BX)    };
!(BX)    controls {
!(BX)         unmanaged XmBulletinBoardDialog finestraFB;
!(BX)    };
!(BX)    callbacks {
!(BX)    };
!(BX)};

object finestraFB : XmBulletinBoardDialog widget {
    arguments {
        XmNautoUnmanage = false;
        XmNresizePolicy = XmRESIZE_GROW;
        XmNdialogTitle = compound_string("FB");
        XmNx = 159;
        XmNy = 403;
    };
    controls {
         managed XmScrollBar scrollBar;
         managed XmPushButton pushButton6;
         managed XmPushButton pushButton5;
    };
    callbacks {
        MrmNcreateCallback = procedure BxGlobalWidgetId(BXG_finestraFB);
    };
};
...
object scrollBar : XmScrollBar widget {
    arguments {
        XmNorientation = XmHORIZONTAL;
        XmNx = 100;
        XmNy = 120;
...
!(BX)    };
!(BX)    controls {
!(BX)         unmanaged XmBulletinBoardDialog finestraFA;
!(BX)    };
!(BX)    callbacks {
!(BX)    };
!(BX)};

object finestraFA : XmBulletinBoardDialog widget {
    arguments {
        XmNautoUnmanage = false;
        XmNresizePolicy = XmRESIZE_GROW;
        XmNdialogTitle = compound_string("FA");
        XmNx = 226;
        XmNy = 208;
        XmNwidth = 306;
        XmNheight = 181;
    ...
    callbacks {
        MrmNcreateCallback = procedure BxGlobalWidgetId(BXG_finestraFA);
    };
};
...
!(BX)    };
!(BX)    controls {
!(BX)         managed XmBulletinBoard bulletinBoard;
!(BX)    };
!(BX)    callbacks {
!(BX)    };
!(BX)};
...
object pushButton : XmPushButton widget {
    ...
    callbacks {
        XmNactivateCallback = procedure AttivaFinestra("A");
    };
};

object pushButton1 : XmPushButton widget {
    arguments {
        XmNlabelString = compound_string("B\r");
        XmNx = 160;
        XmNy = 100;
        XmNwidth = 107;
        XmNheight = 38;
    ...
    callbacks {
        XmNactivateCallback = procedure AttivaFinestra("B");
    };
};
...
object pushButton2 : XmPushButton widget {
    ...
    callbacks {
        XmNactivateCallback = procedure AttivaFinestra("C");
    };
};
