...
char comando[100];
char risposta[100];
char client_msg[ BUF_SIZE ], device_msg[ BUF_SIZE ], alarm_msg[ BUF_SIZE ], alarm_headmsg[ NCHARS ], myname[ NCHARS ] ;
...
char **argv ;
{
    ...
    //extern char client_msg[ BUF_SIZE ], device_msg[ BUF_SIZE ], alarm_msg[ BUF_SIZE ], myname[ NCHARS ] ;
...
ipcfile = fopen("ipcfile", "a");
fprintf(ipcfile, "sudo ipcrm -m %d\n", command_shame_id );
fclose(ipcfile);
chmod("ipcfile", S_IRWXU | S_IRWXG | S_IRWXO );
...
ipcfile = fopen("ipcfile", "a");
fprintf(ipcfile, "sudo ipcrm -s %d\n", command_sem_id );
fclose(ipcfile);
chmod("ipcfile", S_IRWXU | S_IRWXG | S_IRWXO );
...
printf("\n\n\n\nP FROM GRAPHICS == >>%s\n", client_msg);
		...
		|| strncmp ( client_msg+1, "wr", 2 ) == 1 || strncmp ( client_msg+1, "rd", 2 ) == 1 )
			...
			do	{
				// ritardo per aspettare che si liberi la linea seriale
				////////////////////////////////////////////////////////////////////
				...
				printf("PBM letto da mem: %s\n", risposta);
				usleep(100000); // microsecondi
				} while  ( strcmp(risposta, "busy" ) == 0 );
			...
			usleep(200000); // microsecondi
                    ...
                    /* answer to client */
                    do {
                        write_retval = write( main_channel, risposta, BUF_SIZE ) ;
                        if ( write_retval <= 0 )
                            printf( "\n%s: error writing main socket\n",
                                    myname ) ;
                        } while ( write_retval <= 0 ) ;
				...
				printf("\n\n\n\nPT TO GRAPHICS == >>%s\n", "Done");
                    				...
                    				/* answer to client */
                    				do {
                        				write_retval = write( main_channel, "Done", BUF_SIZE ) ;
                        				if ( write_retval <= 0 )
                            				printf( "\n%s: error writing main socket\n",
                                    				myname ) ;
                        				} while ( write_retval <= 0 ) ;
		...
		do
			...
			if ( comando[0] != 0)
				...
				printf("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF answer is %s\n", device_msg);
				...
				////////////////////////////////////////////////////////////////////
        			...
        			else 
					{
       					//  ??  serve ??  answer_sem_val = semctl( answer_sem_id, 0, GETVAL, NULL ) ;
					//strcpy(answer_shame_pointer, "done");
					strcpy(answer_shame_pointer, device_msg);
					}
